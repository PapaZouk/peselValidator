...
public class PeselGeneratorNineteenthCentury implements PeselGenerator {
    ...
    public String generatePeselWithMonthAndGender(int monthStart, int monthEnd, Gender gender) {
        return generatePesel(1, 31, monthStart, monthEnd, 1, 12, gender);
    }
    ...
    public String generatePesel(int dayStart, int dayEnd, int monthStart, int monthEnd, int yearStart, int yearEnd, Gender gender) {
        ...
        if (monthStart <= 0 || monthEnd > 12) {
            throw new IllegalArgumentException("Invalid month value. Acceptable values are 1 to 12");
        }
        ...
        LocalDate date = DateGenerator.generateRandomDate(
                ...
                yearStart,
                yearEnd + 1,
                PeselGeneratorType.NINETEENTH_CENTURY);
        String PESEL = DateToPeselParser.parseDateToPesel(date, PeselGeneratorType.NINETEENTH_CENTURY, gender);
